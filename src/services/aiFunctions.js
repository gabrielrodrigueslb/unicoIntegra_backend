...
/**
 * Substitui todas as ocorrências de {{variavel}} dentro de um JSON,
 * incluindo strings aninhadas.
 */
function applyTemplate(obj, vars) {
  ...
  if (Array.isArray(obj)) {
    return obj.map((v) => applyTemplate(v, vars));
  }

  if (typeof obj === "object" && obj !== null) {
    const newObj = {};
    for (const [k, v] of Object.entries(obj)) {
      newObj[k] = applyTemplate(v, vars);
    }
    return newObj;
  }
...
export async function alpha7Functions(
  ...
  const payloadEnvioItemsTemplate = {
   "name":"Envio de Itens ",
   "type":2,
   "version":1,
 ...
 const vars = { dbName, apiKey, instance, queueId };

  // 🔹 Aplica as substituições no template
  const payloadEnvioItems = applyTemplate(payloadEnvioItemsTemplate, vars);
  // =====================================================================
  // EXECUÇÃO
  // =====================================================================
  try {
    console.log('Iniciando instalação: Adiciona Item...');
    const adicionaItemResponse = await axios.post(
      `${instance}/ivrs/`,
      payloadAdicionaItem,
      axiosConfig,
    );

    if (!adicionaItemResponse.data?.id)
      throw new Error('Resposta inválida ao criar Adiciona Item');
    console.log('Adiciona Item criado. ID:', adicionaItemResponse.data.id);

    console.log('Iniciando instalação: Envio de Itens...');
    const envioItemsResponse = await axios.post(
      `${instance}/ivrs/`,
      payloadEnvioItems,
      axiosConfig,
    );

    if (!envioItemsResponse.data?.id)
      throw new Error('Resposta inválida ao criar Envio de Itens');
